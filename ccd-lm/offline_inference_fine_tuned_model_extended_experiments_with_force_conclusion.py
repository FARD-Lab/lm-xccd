...
from analyse_force_conclusion import Analyser as ForceAnalyser
...
class ForceConclusionOflineRequest(OfflineRequest):

    print("Inferencing using Phi3 Model with Force Conclusion. Begins .......")
    ...
    def _send_request(self, prompt_id, prompt): 
        prompt_id, response = super()._send_request(prompt_id, prompt)
        final_conclusion = self._force_final_conclusion(response)
        print(f"[{prompt_id}] FINAL FORCED CONCLUSION = {final_conclusion}\n")

        return prompt_id, response, final_conclusion
...
class CodeCloneDetectionPhiForceConclusion(CodeCloneDetection):
    def __init__(
        self,
        data_file: str,
        temperature: float = 0.7,
        ...
        super().__init__(data_file, temperature, model, output_file)
        self.gpt = ForceConclusionOflineRequest(model=self.model, stream=False)
...
def main():
    ...
    parser.add_argument("-fo", "--force", help="Force Conclusion")
    ...
    suffix = "_force_conclusion" if args.force else ""
    output = os.path.join(current_location, f"{args.datafile}_{args.model}{suffix}_inference_result.jsonl")
    ...
    if args.finetuned:
        ...
        print("Running Force Conclusion version")
        ccd = CodeCloneDetectionPhiForceConclusion(
            model=local_models[args.model],
            data_file=data_file_path,
            output_file= output
        )
    ...
    if args.force:
        analyser = ForceAnalyser(
            data_file_path,
            output
        )
        description=f"{metric_description_name}_{args.model}_force_conclusion_evaluation_result"
    else:
        analyser = Analyser(
            data_file_path,
            output
