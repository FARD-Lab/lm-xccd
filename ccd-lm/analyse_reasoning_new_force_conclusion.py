...
class Analyser:
    ...
    def extract_llm_result(self, text, conclusion):
        ...
        if idx or idx2:
            ...
            # if conclusion.to_lower == 'yes':
            #     return 1
            # if conclusion.to_lower == 'no':
            #     return 0  
    ...
    def compute_metrics(self, description = None, save_to_file=False):
        ...
        for element in self.ground_truth:
            ...
            if key in self.predicted_results:
                ...
                # prediction_fake_label = 0 if element[key] == 1 else 1
                # predictions_labels.append(prediction_fake_label)
                # ground_truth_labels.append(element[key])
...
if __name__ == '__main__':
    ...
    # analyser = Analyser(
    #     os.path.join(current_location, 'extended-experiments/train-test-data/train_data_clones.jsonl'),
    ...
    # analyser = Analyser(
    #     os.path.join(current_location, 'extended-experiments/train-test-data/train_data_clones.jsonl'),
    #     os.path.join(current_location, 'offline_results', 'test-different-dist-qwen-main-prompt-force-conclusion.jsonl')
    # )
    # analyser.compute_metrics("Same Distribution Test All Clones Qwen pure results force conclusion ", True)
    # analyser.compute_missing_samples(type='same_dist_deepseek')
    # analyser = Analyser(
    #     os.path.join(current_location, 'extended-experiments/train-test-data/train_data_clones.jsonl'),
    #     os.path.join(current_location, 'offline_results', 'test-same-dist-main-prompt-finetuned-version2-force-conclusion.jsonl')
    # )
    # analyser.compute_metrics("Same Distribution Test All Clones phi3 version2 results force conclusion ", True)
    # analyser.compute_missing_samples(type='same_dist_deepseek')
    ...
    #     os.path.join(current_location, 'offline_results', 'test-same-dist-main-prompt-finetuned-version2-force-conclusion.jsonl')
    # )
    # analyser.compute_metrics("Same Distribution Test All Clones Phi3 version2 force conclusion ", True)
    # analyser.compute_missing_samples(type='same_dist_phi3')

    # analyser = Analyser(
    #     os.path.join(current_location, 'extended-experiments/train-test-data/same_distribution_test_clones.jsonl'),
    #     os.path.join(current_location, 'offline_results', 'test-same-dist-main-prompt-force-conclusion.jsonl')
    # )
    ...
    #     os.path.join(current_location, 'offline_results', 'test-different-dist-qwen-main-prompt-force-conclusion.jsonl')
    # )
    # analyser.compute_metrics("Different Distribution Test All Clones qwen force conclusion ", True)
    # analyser.compute_missing_samples(type='different_dist_qwen')
    
    # analyser = Analyser(
    #     os.path.join(current_location, 'extended-experiments/train-test-data/same_distribution_test_clones.jsonl'),
    #     os.path.join(current_location, 'offline_results', 'test-same-dist-qwen-main-prompt-force-conclusion.jsonl')
    # )
    # analyser.compute_metrics("Same Distribution Test All Clones Qwen pure results force conclusion ", True)
    # analyser.compute_missing_samples(type='same_dist_phi3')
    ...
    # )
    # analyser.compute_metrics("Different Distribution Test All Clones Phi3 force conclusion ", True)
    # analyser.compute_missing_samples(type='different_dist_qwen')
    ...
    # analyser = Analyser(
    #     os.path.join(current_location, 'extended-experiments/train-test-data/different_distribution_test_clones.jsonl'),
    #     os.path.join(current_location, 'offline_results', 'classifier-head-results', 'test-different-distribution-phi3-binaryhead.jsonl')
    # )
    # analyser.compute_metrics("Different Distribution  Phi3 Binary head ", True)
    # analyser.compute_missing_samples(type='different_dist_phi3_binary_head')

    # analyser = Analyser(
    #     os.path.join(current_location, 'extended-experiments/train-test-data/different_distribution_test_clones.jsonl'),
    #     os.path.join(current_location, 'offline_results', 'classifier-head-results', 'test-different-distribution-phi3-contrastivehead.jsonl')
    # )
    # analyser.compute_metrics("Different Distribution  Phi3 contrastive head ", True)
    # analyser.compute_missing_samples(type='different_dist_phi3_contrastive_head')

    # analyser = Analyser(
    #     os.path.join(current_location, 'extended-experiments/train-test-data/same_distribution_test_clones.jsonl'),
    #     os.path.join(current_location, 'offline_results', 'classifier-head-results', 'test-same-distribution-phi3-binaryhead.jsonl')
    # )
    # analyser.compute_metrics("Same Distribution Test phi3 binary head ", True)
    # analyser.compute_missing_samples(type='same_dist_phi3_binary_head')

    analyser = Analyser(
        os.path.join(current_location, 'extended-experiments/train-test-data/same_distribution_test_clones.jsonl'),
        os.path.join(current_location, 'offline_results', 'classifier-head-results', 'test-same-distribution-phi3-contrastivehead.jsonl')
    )
    analyser.compute_metrics("Same Distribution Test phi3 contrastive head ", True)
    analyser.compute_missing_samples(type='same_dist_phi3_contrastive_head')